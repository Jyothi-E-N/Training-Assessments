...
from banking.accounts.account_types.current_account import CurrentAccount
from banking.accounts.account_types.savings_account import SavingsAccount
from banking.accounts.account_types.overdraft_account import OverdraftAccount
...
class Bank:
    ...
    def open_account(self, name, password, balance, account_type='current'):
        self._last_id+=1

        ## change the below code
        if(account_type=='overdraft'):
            account=OverdraftAccount(self._last_id, name, password, balance)
        ## change the above code

        elif(account_type=='savings'):
            # min_balance could be taken user input
            min_balance=5000
            account=SavingsAccount(min_balance, self._last_id, name, password, balance)
        else:
            account=CurrentAccount(self._last_id, name, password, balance)
